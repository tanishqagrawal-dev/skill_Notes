...
window.AIGenerator = {
    ...
    getPaper: async (subjectId, subjectName, examType, syllabusContext = "", uid = "guest") => {
        ...
        try {
            ...
            try {
                ...
                throw new Error(`AI generation failed (${e.message}) and no cached papers found.`);
            }
        }
    },
    ...
    callGemini: async (subject, type, syllabus) => {
        try {
            console.log("✨ SKiL MATRiX AI: Requesting generation from secure server...");
            
            const res = await fetch('/api/ai/generate-model-paper', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ subject, type, syllabus })
            });
